...
#let example(title, image-code, width, subtitle: none, should-index: true, c: example-counter) = context {
  box(width: 100%, {
    c.step()
    enum(
    ...
    align(center)[#image("figures/fig_" + ("0000000" + str(image-code)).slice(-3) + ".jpg", width: width)]
  })
}
...
이 책 전반에서, 화음은 으뜸음#sub[tonic]$$(또는 조성 중심#sub[tonal center])과의 관계를 기준으로 설명된다. 이 관계를 나타내기 위해 로마 숫자가 사용됩니다. 장조의 화성을 다룰 때, 화음은 기본적으로 장음계#sub[major scale]에서 파생된 것으로 간주된다. 반면, 단조의 화성을 다룰 때는 화음이 #sans[화성] 단음계#sub[_harmonic_ minor scale]에서 파생된 것으로 간주된다. 다른 단음계도 존재하지만, 이 책에서는 화성 단음계만 논의한다. 화성 단음계는 3도와 6도가 낮아져 있지만, 이끎음#sub[leading tone]은 유지되어 5도 음 위에 형성된 화음이 딸림화음#sub[dominant chord]가 되도록 한다. 이 7도 음은 또한 3도와 7도 음 위에 형성된 화음에도 영향을 미친다.
...
#example("찰리 파커", 044, 41%)

셋잇단음은 이와 유사한 패턴에 리듬적 흥미를 제공한다.

#example("찰리 파커", 045, 41%)
...
#example("덱스터 고든", 054, 41%)

#example("프레디 허버드", 055, 50%)

소니 스팃은 #chord("ii") 화음에서 상행하는 음계로 시작해서 #chord("V7") 화음 위에서 순수한 윤곽을 연주한다.

#example("소니 스팃", 056, 50%)
...
#example("애슐리 알렉산더", 058, 69%)

#example("애슐리 알렉산더", 059, 56%)
...
#example("찰리 파커", 075, 32%)

#example("찰리 파커", 076, 56%)
...
#example("쳇 베이커", 081, 41%)

#example("쳇 베이커", 082, 44%)

#example("쳇 베이커", 083, 62%)
...
#example("J. S. 바흐", 092, 90%, subtitle: [영국 모음곡 #note("G")단조 중 프렐류드])

#example("마이크 스턴", 093, 90%)
...
#example("프레디 허버드", 102, 57%)

#example("클리포드 브라운", 103, 45%)
...
#example("클리포드 브라운", 126, 59%)
#example("케니 배런", 127, 41%)

다음은 브라운의 또 다른 두 개의 예시이다.

#example("클리포드 브라운", 128, 44%)

#example("클리포드 브라운", 129, 68%)

도약은 다음 두 예시를 흥미롭게 만든다. 에반스는 #chord("V7") 화음의 $flat 9$으로 도약하며, 캐넌볼은 #chord("ii") 화음의 3도에서 9도로 도약한다.

#example("빌 에반스", 130, 68%)

#example("캐넌볼 애덜리", 131, 44%)
...
#example("클리포드 브라운", 135, 56%)

#example("소니 롤린스", 136, 56%)
...
#example("부커 리틀", 141, 57%)

#example("부커 리틀", 142, 57%)
...
#example("밀트 잭슨", 153, 57%)

#example("밀트 잭슨", 154, 57%)
...
#example("빌 에반스", 159, 41%)

#example("캐넌볼 애덜리", 160, 69%)
...
#example("텔로니어스 몽크", 174, 32%)

#example("웨스 몽고메리", 175, 32%)

다음은 여러 아티스트가 연주한 단순한 형태의 제2윤곽이다.

#example("클리포드 브라운", 176, 41%)

#example("클리포드 브라운", 177, 28%)

#example("폴 챔버스", 178, 41%)

#example("소니 스팃", 179, 41%)

#example("존 콜트레인", 180, 41%)

#example("톰 하렐", 181, 32%)

이 둘은 반음계적 경과음을 통해 #chord("V7") 화음의 7도까지 하강한다.

#example("찰리 파커", 182, 44%)

#example("톰 하렐", 183, 44%)

다음은 단순한 형태로 제시된 제2윤곽의 추가 예시들이다.

#example("클리포드 브라운", 184, 47%)

#example("빌 에반스", 185, 44%)

#example("캐넌볼 애덜리", 186, 44%)

#example("톰 하렐", 187, 56%)

예시 137의 톰 하렐처럼 애덜리는 #chord("ii") 화음을 미리 예고한다.

#example("캐넌볼 애덜리", 188, 56%)

파커는 목표음 #note("B")를 겨냥하기 전에 아르페지오를 7도 너머 9도까지 확장하여 두 번째 마디의 세 번째 박자에 목표음에 도달한다.

#example("찰리 파커", 189, 57%)
...
#example("지미 히스", 190, 44%)

#example("아트 파머", 191, 44%)
...
#example("클리포드 브라운", 201, 44%)

#example("클리포드 브라운", 202, 53%)

#example("리 모건", 203, 59%)

#example("소니 스팃", 204, 53%)

#example("패츠 나바로", 205, 44%)
...
#example("캐넌볼 애덜리", 210, 69%)

두 개의 온음계 음이 추가된 경우:

#example("클리포드 브라운", 211, 56%)

하위 이웃음이 추가된 경우:

#example("클리포드 브라운", 212, 57%)

#example("톰 하렐", 213, 57%)

도피음#sub[escape tone]이 추가된 경우:

#example("빌 에반스", 214, 60%)

상하위 이웃음으로 둘러싸는 경우:

#example("캐넌볼 애덜리", 215, 44%)
...
#example("클리포드 브라운", 218, 44%)

#example("클리포드 브라운", 219, 57%)
...
#example("톰 하렐", 223, 50%, subtitle: [Amazon])
...
#example("클리포드 브라운", 225, 60%)

#example("조세프 자비눌", 226, 69%)

#example("클리포드 브라운", 227, 90%)

자비눌은 #prog("ii", "V") 진행 위에서 제2윤곽을 연주한 후 #prog("iii", "V7/ii") 위에서 제1윤곽을 연주한다.

#example("조세프 자비눌", 228, 69%)
...
#example("클리포드 브라운", 232, 44%)

#example("빌 에반스", 233, 44%)
...
#example("블루 미첼", 240, 57%)

#example("프레디 허버드", 241, 56%)
...
#example("빌 에반스", 244, 32%)

#example("소니 롤린스", 245, 50%)
...
#example("클리포드 브라운", 247, 41%)

#example("클리포드 브라운", 248, 41%)
...
#example("캐넌볼 애덜리", 266, 50%)

#example("톰 하렐", 267, 44%)

#example("클리포드 브라운", 268, 56%)

#example("클리포드 브라운", 269, 44%)

#example("리 모건", 270, 53%)

#example("찰리 파커", 271, 53%)

#example("프레디 허버드", 272, 59%)
...
#example("덱스터 고든", 273, 44%)

#example([전형적인 라틴 피아노 오스티나토#footnote[동일한 선율의 저음 악구를 계속해서 반복하는 음악적 기법]], 274, 45%, should-index: false)
...
#example("찰리 파커", 281, 56%)

#example("찰리 파커", 282, 44%)

#example("클리포드 브라운", 283, 44%)

#example("캐넌볼 애덜리", 284, 44%)

이 선율은 두 딸림7화음 모두에서 3도에서 근음으로 도약하는 대칭성을 가진다. 제3윤곽은 #chord("Fm7") 이전에 시작되지만 #chord("Bb7")에 제시간에 해결된다.

#example("클리포드 브라운", 285, 44%)

캐넌볼은 동일한 솔로에서 두 개의 조성 영역에서 동일한 선율을 사용한다.

#example("캐넌볼 애덜리", 286, 32%)

#example("캐넌볼 애덜리", 287, 45%)
...
#example("랜디 브레커", 306, 44%, subtitle: [_Passenger_])
#example("랜디 브레커", 307, 56%, subtitle: [_Passenger_])
...
#example("캐넌볼 애덜리", 308, 44%)
#example("캐넌볼 애덜리", 309, 44%)
...
#fb[7. 둘러싸기, 아르페지오 음, 반음계적 접근음. 첫 마디의 D에서 F로 하행 도약이 마지막 마디에서 반복된다.
...
#fb[2. 제2윤곽을 사용하기. 첫 절반에서는 홀수 마디에, 후반부에서는 짝수 마디에 윤곽이 시작된다
#align(center)[#image("figures/fig_378.jpg", width: 90%)]]
#fb[3. 제3윤곽을 사용하기. 첫 절반에서는 홀수 마디에 윤곽이 시작하고, 후반부에서는 윤곽이 전위된다
